...
namespace Junction\Api\Test\Unit\Http\Middleware\DestinationLog;

use Georgeff\Bus\DispatcherInterface;
use Junction\Api\DestinationLog\Command\QueryAllForDestination;
use Junction\Api\Http\Middleware\DestinationLog\AllForDestination;
...
final class AllForDestinationTest extends TestCase
{
    ...
    public function test_dispatches_command_with_id_and_cursor_params(): void
    {
        ...
        $dispatcher->expects($this->once())
            ->method('dispatch')
            ->with($this->callback(
                fn(QueryAllForDestination $cmd) => $cmd->destinationId === 'dest-uuid'
                    && $cmd->limit === 25
                    && $cmd->cursor === null
            ))
        ...
        (new AllForDestination($dispatcher))->process($request, $handler);
    }
    ...
    public function test_passes_cursor_token_to_command(): void
    {
        ...
        $dispatcher->expects($this->once())
            ->method('dispatch')
            ->with($this->callback(fn(QueryAllForDestination $cmd) => $cmd->cursor === 'tok123'))
            ->willReturn($this->makePaginator());
    ...
    public function test_sets_result_as_data_attribute_and_delegates(): void
    {
        ...
        $result = (new AllForDestination($dispatcher))->process($request, $handler);
    ...
    public function test_assert_fails_when_cursor_params_not_set(): void
    {
        ...
        (new AllForDestination($this->createMock(DispatcherInterface::class)))->process(
            $request,
